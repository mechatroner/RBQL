...
async function test_json_tables() {
    ...
    for (let test_case of tests) {
        ...
        let error_type = null;
        try {
            await rbql.query_table(query, input_table, output_table, warnings, join_table, input_column_names, join_column_names, output_column_names, user_init_code);
        } catch (e) {
            if (local_debug_mode)
                throw(e);
        ...
        test_common.assert(expected_error === null, `Expected error was not triggered: "${expected_error}".`)
